...
function validateTicket(ticket: unknown, filePath: string): ticket is Ticket {
  try {
    ...
    core.error(`Error validating ticket in ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
    return false;
  }
}
    ...
    for (const filePath of yamlFiles) {
      ...
      try {
        ...
        try {
          ...
          logger.info(`Processing: ${ticketSummary.title || 'Untitled Ticket'}`, '📋');
...
function generateIssueBody(ticket: Ticket): string {
  ...
  if (ticket.sub_tasks?.length) {
    body += '## Sub-tasks\n'
    body += ticket.sub_tasks.map((task, index) => `${index + 1}. ${task}`).join('\n')
    body += '\n\n'
  }
  ...
  // Add metadata
  const metadata = []
  if (ticket.estimated_time) metadata.push(`**Estimated Time:** ${ticket.estimated_time}`)
  if (ticket.complexity) metadata.push(`**Complexity:** ${ticket.complexity}`)
  if (ticket.change_impact) metadata.push(`**Change Impact:** ${ticket.change_impact}`)
...
function convertLegacyFormat(legacyTicket: Record<string, unknown>): Ticket {
  ...
  if (typeof legacyTicket.Description === 'string') ticket.description = legacyTicket.Description
  
  // Handle acceptance criteria
  if (Array.isArray(legacyTicket['Acceptance Criteria'])) {
    ticket.acceptance_criteria = legacyTicket['Acceptance Criteria'].filter((item: unknown): item is string => typeof item === 'string')
  ...
  if (legacyTicket['Technical Details']) {
    ...
    if (typeof legacyTicket['Technical Details'] === 'object' && legacyTicket['Technical Details'] !== null) {
      Object.assign(ticket.technical_details, legacyTicket['Technical Details'])
    }
  }
  
  // Handle sub-tasks
  if (Array.isArray(legacyTicket['Sub-tasks'])) {
    ticket.sub_tasks = legacyTicket['Sub-tasks'].filter((item: unknown): item is string => typeof item === 'string')
  }
  
  // Handle dependencies
  if (Array.isArray(legacyTicket.Dependencies)) {
    ticket.dependencies = legacyTicket.Dependencies.filter((item: unknown): item is string => typeof item === 'string')
  } else if (typeof legacyTicket.Dependencies === 'string') {
    ticket.dependencies = [legacyTicket.Dependencies]
  }
  
  // Copy other fields
  if (typeof legacyTicket['Estimated Time'] === 'string') ticket.estimated_time = legacyTicket['Estimated Time']
  if (typeof legacyTicket.Complexity === 'string') ticket.complexity = legacyTicket.Complexity
  if (typeof legacyTicket['Change Impact'] === 'string') ticket.change_impact = legacyTicket['Change Impact']
